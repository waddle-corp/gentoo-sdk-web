import './floating-sdk-cafe24-sidepanel.css';
...
class FloatingButton {
    constructor(props) {
        ...
        this._bindedUnloadHandler = () => {
            try { this.restoreParentLayout(); } catch (_) {}
        };
        window.addEventListener('beforeunload', this._bindedUnloadHandler);
    ...
    remove() {
        // Always restore parent layout when elements are removed
        try { this.restoreParentLayout(); } catch (_) {}
        if (this.button) {
        ...
        if (this._bindedUnloadHandler) {
            window.removeEventListener('beforeunload', this._bindedUnloadHandler);
            this._bindedUnloadHandler = null;
        }
        this.button = null;
    ...
    destroy() {
        ...
        // Restore any parent layout shifts
        try { this.restoreParentLayout(); } catch (_) {}
    ...
    // Restore any page-level style changes applied when opening the side panel
    restoreParentLayout() {
    ...
    enableChat(mode) {
        ...
        // Open as right side panel and push page
        if (mode === "shrink" || mode === "full") {
            this.iframeContainer.className = "iframe-container";
            if (this.isMobileDevice) this.iframeContainer.style.height = "100%";

            // push content left by adding right margin on html root
            try {
                const html = document.documentElement;
                const body = document.body;
