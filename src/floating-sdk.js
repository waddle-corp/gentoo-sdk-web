...
class FloatingButton {
    ...
    createUIElements(position, showGentooButton, isCustomButton = false, parentElem, emergeThreshold) {
        ...
        if (showGentooButton) {
            ...
            if (this.partnerId === '67615284c5ff44110dbc6613') {
                if (this.floatingZoom && !this.partnerId === '67615284c5ff44110dbc6613') {
            ...
            // tmp for fastfive
            if (this.partnerId === '67615284c5ff44110dbc6613') this.buttonContainer.classList.add('button-margin-left');
        ...
        // Add event listeners
        this.setupEventListeners(position, isCustomButton, emergeThreshold);
    ...
    setupEventListeners(position, isCustomButton, emergeThreshold) {
        ...
        var buttonClickHandler = (e) => {
            ...
            if (this.iframeContainer.classList.contains("iframe-container-hide")) {
                ...
                if (this.button) {
                    if (this.isSmallResolution) {
                        this.button.className = `floating-button-common ${this.floatingZoom && !this.partnerId === '67615284c5ff44110dbc6613' ? 'button-zoom' : 'button-md'}`;
                    } else {
            ...
            return () => {
                ...
                requestAnimationFrame(() => {
                    const y = window.scrollY
                        ...
                        || 0;
                    this.updateFloatingContainerVisibility(y < document.documentElement.scrollHeight * emergeThreshold?.scrollThreshold ? 'none' : 'flex');
                    if (this.floatingContainer && this.floatingContainer.parentNode && !this.gentooSessionData.floatingEmerged) {
                        this.gentooSessionData.floatingEmerged = true;
                        sessionStorage.setItem('gentoo', JSON.stringify(this.gentooSessionData));
                        this.floatingContainer.appendChild(this.expandedButtonWrapper);
                        setTimeout(() => {
                            this.addLetter(this.bootConfig, this.expandedText, () => this.isDestroyed);
        ...
        const onTime = (() => {
            ...
            return () => {
                ...
                ticking = true;
                setTimeout(() => {
                    this.updateFloatingContainerVisibility('flex');
                    if (this.floatingContainer && this.floatingContainer.parentNode && !this.gentooSessionData.floatingEmerged) {
                        this.gentooSessionData.floatingEmerged = true;
                        sessionStorage.setItem('gentoo', JSON.stringify(this.gentooSessionData));
                        this.floatingContainer.appendChild(this.expandedButtonWrapper);
                ...
                }, emergeThreshold?.timeThreshold);
            };
        })();

        if (emergeThreshold?.isScrollBased) window.addEventListener('scroll', onScroll, { passive: true });
        else if (emergeThreshold?.isTimeBased) onTime();
    ...
    updateFloatingContainerVisibility(visibility = 'flex') {
        if (this.floatingContainer) {
            this.floatingContainer.classList.toggle('hide-visibility', visibility === 'none');
        }
    }
    ...
    deleteViewport() {
        ...
        try {
            ...
            if (this.injectedViewport && this.injectedViewport.parentNode) {
                this.injectedViewport.parentNode.removeChild(this.injectedViewport);
            } else {
